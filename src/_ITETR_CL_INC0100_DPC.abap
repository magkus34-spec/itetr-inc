class /ITETR/CL_INC0100_DPC definition
  public
...
protected section.
  ...
  methods LIFNRSHSET_CREATE_ENTITY
    ...
    exporting
      !ER_ENTITY type /ITETR/CL_INC0100_MPC=>TS_LIFNRSH
    raising
  ...
  methods LIFNRSHSET_GET_ENTITYSET
    ...
    exporting
      !ET_ENTITYSET type /ITETR/CL_INC0100_MPC=>TT_LIFNRSH
...
CLASS /ITETR/CL_INC0100_DPC IMPLEMENTATION.


  method /IWBEP/IF_MGW_APPL_SRV_RUNTIME~CREATE_ENTITY.
*&----------------------------------------------------------------------------------------------*
*&  Include           /IWBEP/DPC_TEMP_CRT_ENTITY_BASE
*&* This class has been generated on 13.04.2023 13:58:39 in client 100
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - /ITETR/CL_INC0100_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA lifnrshset_create_entity TYPE /itetr/cl_inc0100_mpc=>ts_lifnrsh.
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA lifnrshset_get_entity TYPE /itetr/cl_inc0100_mpc=>ts_lifnrsh.
 DATA lv_entityset_name TYPE string.
...
*&*   generated methods inside the DPC provider subclass - /ITETR/CL_INC0100_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA lifnrshset_get_entityset TYPE /itetr/cl_inc0100_mpc=>tt_lifnrsh.
 DATA lv_entityset_name TYPE string.
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA lifnrshset_update_entity TYPE /itetr/cl_inc0100_mpc=>ts_lifnrsh.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
DATA lr_KTOKK LIKE RANGE OF ls_converted_keys-ktokk.
DATA ls_KTOKK LIKE LINE OF lr_KTOKK.
...
DATA lr_BEGRU LIKE RANGE OF ls_converted_keys-begru.
DATA ls_BEGRU LIKE LINE OF lr_BEGRU.
...
CASE ls_filter-property.
       WHEN 'KTOKK'.              " Equivalent to 'Ktokk' property in the service
    lo_filter->convert_select_option(
      EXPORTING
        is_select_option = ls_filter
      IMPORTING
        et_select_option = lr_KTOKK ).

    LOOP AT lr_KTOKK INTO ls_KTOKK.
             ls_selopt-sign = ls_KTOKK-sign.
        ls_selopt-option = ls_KTOKK-option.
        ls_selopt-low = ls_KTOKK-low.
        ls_selopt-high = ls_KTOKK-high.
      ls_selopt-shlpfield = 'KTOKK'.
      ls_selopt-shlpname = '/ITETR/INC_SH_LIFNR'.
    ...
    LOOP AT lr_LIFNR INTO ls_LIFNR.
      ...
      CLEAR ls_selopt.
      ENDLOOP.
       WHEN 'BEGRU'.              " Equivalent to 'Begru' property in the service
    lo_filter->convert_select_option(
      EXPORTING
        is_select_option = ls_filter
      IMPORTING
        et_select_option = lr_BEGRU ).

    LOOP AT lr_BEGRU INTO ls_BEGRU.
             ls_selopt-sign = ls_BEGRU-sign.
        ls_selopt-option = ls_BEGRU-option.
        ls_selopt-low = ls_BEGRU-low.
        ls_selopt-high = ls_BEGRU-high.
      ls_selopt-shlpfield = 'BEGRU'.
      ls_selopt-shlpname = '/ITETR/INC_SH_LIFNR'.
      APPEND ls_selopt TO lt_selopt.
      CLEAR ls_selopt.
      ENDLOOP.
...
* Get search help values
  me->/iwbep/if_sb_gendpc_shlp_data~get_search_help_values(
    EXPORTING
      iv_shlp_name = '/ITETR/INC_SH_LIFNR'
 ...
 CLEAR et_entityset.
  ...
  LOOP AT lt_result_list INTO ls_result_list
    ...
    CASE ls_result_list-field_name.
      WHEN 'BEGRU'.
        ls_entityset-begru = ls_result_list-field_value.
      WHEN 'KTOKK'.
        ls_entityset-ktokk = ls_result_list-field_value.
      WHEN 'LAND1'.
